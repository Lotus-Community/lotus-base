...
- The GUI is now in a separate submodule repository from chia-blockchain, [lotus-blockchain-gui](https://github.com/Chia-Network/lotus-blockchain-gui). The installers and install scripts have been updated and it continues to follow the same install steps. Note that the GUI directory will now be `lotus-blockchain-gui`. The workflow for this may be "touch and go" for people who use the git install methods over the short term.
